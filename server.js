...
app.post('/api/client-auth/login', ClientAuth.csrf, wrap(async (req, res) => {
  ...
  const { rows } = await pool.query(`
    SELECT ca.id, ca.client_id, ca.email, ca.password_hash, ca.is_active, ca.must_change_password,
           c.full_name
...
app.delete('/api/clients/:id/constraints/:constraintId', A.requireCsrfHeader, A.requireRole('owner','clinician'), wrap(async (req, res) => {
  if (!await assertOwnsClient(req.params.id, req.user.id)) return res.status(404).json({ error: 'not_found' });
